...
class SimpleOverlayRenderer:
    ...
    def create_template4_overlay(
        ...
        """
        Create Template 4 (Tiruvarur Updates) overlay using Pillow.
        ...
        try:
            ...
            # Load fonts
            try:
                # Try to load Arial Bold for headline
                font_headline = ImageFont.truetype("C:/Windows/Fonts/arialbd.ttf", 52)
                font_logo = ImageFont.truetype("C:/Windows/Fonts/arialbd.ttf", 28)
                font_date = ImageFont.truetype("C:/Windows/Fonts/arialbd.ttf", 36)
                font_location = ImageFont.truetype("C:/Windows/Fonts/arialbd.ttf", 38)
            except:
            ...
            # 1. Draw dark red border
            border_color = (92, 26, 26, 255)  # #5c1a1a
            draw.rectangle([0, 0, width-1, height-1], outline=border_color, width=20)

            # 2. Draw red/maroon top section
            top_color = (139, 37, 0, 240)  # #8b2500 with opacity
            draw.rectangle([20, 20, width-20, 300], fill=top_color)

            # 3. Draw gradient overlay (approximation)
            lighter_red = (196, 59, 0, 76)  # #c43b00 with lower opacity
            draw.rectangle([20, 20, width-20, 160], fill=lighter_red)

            # 4. Draw logo box (top right)
            logo_x = width - 250
            logo_y = 60
            logo_bg = (255, 255, 255, 255)
            logo_border = (139, 37, 0, 255)
            draw.rectangle([logo_x, logo_y, logo_x+200, logo_y+80],
                          fill=logo_bg, outline=logo_border, width=4)

            # Draw logo text
            logo_text = "TIRUVARUR\nUPDATES"
            draw.multiline_text((logo_x+100, logo_y+40), logo_text,
                               fill=(139, 37, 0, 255), font=font_logo,
                               anchor="mm", align="center")

            # 5. Draw date (top section, left)
            current_date = datetime.now().strftime("%d-%m-%Y")
            draw.text((80, 220), current_date, fill=(255, 255, 255, 255),
                     font=font_date)

            # 6. Draw headline box (semi-transparent cream background)
            headline_y = height - 600
            headline_bg = (245, 240, 232, 250)  # Cream with opacity
            headline_border = (139, 37, 0, 255)  # Dark red border

            # Draw headline background
            draw.rectangle([60, headline_y, width-60, headline_y+200],
                          fill=headline_bg, outline=headline_border, width=5)

            # Draw headline text (wrapped)
            headline_wrapped = SimpleOverlayRenderer._wrap_text(
                headline, font_headline, width-180
            )
            draw.multiline_text((width//2, headline_y+100), headline_wrapped,
                               fill=(61, 40, 23, 255), font=font_headline,
                               anchor="mm", align="center")

            # 7. Draw location box (if enabled)
            if show_location and location:
                loc_y = height - 240
                loc_bg = (92, 26, 26, 242)  # Dark red with opacity

                # Draw location background (rounded rectangle approximation)
                draw.ellipse([60, loc_y, 120, loc_y+60], fill=loc_bg)
                draw.ellipse([width-120, loc_y, width-60, loc_y+60], fill=loc_bg)
                draw.rectangle([90, loc_y, width-90, loc_y+60], fill=loc_bg)

                # Draw location text with icon
                loc_text = f"📍 {location}"
                bbox = draw.textbbox((0, 0), loc_text, font=font_location)
                text_width = bbox[2] - bbox[0]
                draw.text((width//2 - text_width//2, loc_y+30), loc_text,
                         fill=(255, 255, 255, 255), font=font_location)

            # Save image
            img.save(output_path, 'PNG')
            print(f"✅ Created overlay: {Path(output_path).name}")
            return True
